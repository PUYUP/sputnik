...
from apps.helpdesk.utils.constants import (
    CANAL_CHOICES, TEXT,
    CONSULT_STATUS, OPEN,
    CLASSIFY_CHOICES, CONSULT,
    ASSIGN_STATUS, WAITING, ACCEPT,
    EVENT_CHOICES
)


class AbstractConsult(models.Model):
    ...
    topic = models.ManyToManyField('helpdesk.Topic', related_name='consults',
                                   limit_choices_to={'is_active': True})

    number = models.CharField(max_length=255, editable=False)
    status = models.CharField(choices=CONSULT_STATUS, default=OPEN, max_length=25,
                              validators=[IDENTIFIER_VALIDATOR, non_python_keyword])

    # if :expert give a respond at first time change this to True
    ...
    class Meta:
        ...
        verbose_name = _("Consult")
        verbose_name_plural = _("Consults")
...
class AbstractAssign(models.Model):
    """One :consult only assigned to one :expert
    IMPORTANT! the :assign has attribute name :extra_cost
    with that attribute allow client add more cost for the consult"""
    ...
    # a selected consult
    consult = models.ForeignKey('helpdesk.Consult', on_delete=models.CASCADE,
                                related_name='assigns')
    # target to Expert
    expert = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE,
                             related_name='assign_experts')
    ...
    # maybe client want to assign :consult to another :expert
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['consult', 'expert'], 
                condition=models.Q(status=ACCEPT),
                name='unique_consult_expert'
            ),
        ]

    def __str__(self):
        return self.consult.number

    def save(self, *args, **kwargs):
        self.user = self.consult.user
    ...
    def _validate_status(self, value):
        # status :accpeted by expert can't change again
        if self.pk:
            old = self.__class__.objects.get(id=self.pk)
            has_accepted = self._has_accepted()

            if old.status == ACCEPT:
                raise ValidationError(_("Status Accept can't change again"))

            if has_accepted:
                raise ValidationError(_("Assign has accpeted by %s" % self.expert.username))

    def _has_accepted(self):
        # check consult assign has accepted
        queryset = self.__class__.objects \
            .prefetch_related(Prefetch('user'), Prefetch('consult'), Prefetch('expert')) \
            .select_related('user', 'consult', 'expert') \
            .filter(status=ACCEPT, consult_id=self.consult.id)
...
class AbstractAssigned(models.Model):
    ...
    # who assigned (related as expert)
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE,
    ...
    def save(self, *args, **kwargs):
        self.user = self.assign.expert
        self.consult = self.assign.consult

        if not self.pk:
            slatime = timezone.datetime.strptime(str(self.assign.segment.open_hour), '%H:%M:%S')
            duetime = timezone.timedelta(hours=self.assign.sla.grace_periode) + slatime
            duetime_fmt = '{0}:{1}:{2}'.format(duetime.hour, duetime.minute, duetime.second)
...
class AbstractRespond(models.Model):
    _limit_content_type = models.Q(app_label='helpdesk') & models.Q(model__in=['consult'])
