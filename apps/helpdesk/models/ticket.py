...
class AbstractTicket(models.Model):
    ...
    topic = models.ManyToManyField('helpdesk.Topic', related_name='tickets',
                                   limit_choices_to={'is_active': True})

    number = models.CharField(max_length=255, editable=False)
    status = models.CharField(choices=TICKET_STATUS, default=OPEN, max_length=25,
...
class AbstractAssign(models.Model):
    """One :ticket only assigned to one :expert
    IMPORTANT! the :assign has attribute name :extra_cost
    with that attribute allow client add more cost for the ticket"""
    ...
    # maybe client want to assign :ticket to another :expert
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['ticket', 'expert'], 
                condition=models.Q(status=ACCEPT),
                name='unique_ticket_expert'
            ),
        ]

    def __str__(self):
        return self.ticket.number
    ...
    def _has_accepted(self):
        # check ticket assign has accepted
        queryset = self.__class__.objects \
            .prefetch_related(Prefetch('user'), Prefetch('ticket'), Prefetch('expert')) \
            .select_related('user', 'ticket', 'expert') \
            .filter(status=ACCEPT, ticket_id=self.ticket.id)
...
class AbstractAssigned(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        self.ticket = self.assign.ticket
...
class AbstractRespond(models.Model):
    _limit_content_type = models.Q(app_label='helpdesk') & models.Q(model__in=['ticket'])
